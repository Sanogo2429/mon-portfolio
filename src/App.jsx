...
const App = () => {
  ...
  const colors = [
    { id: 'blue', hex: '#2563eb', label: 'Bleu Royal' },
  ...
  // --- DONNÉES DES PROJETS AVEC LIENS RÉELS ---
  const projects = [
    {
      title: "Serre Intelligente IoT",
      tech: ["C#", "Arduino", "MySQL"],
      desc: "Système d'arrosage autonome piloté par données capteurs (Projet Gaia). Automatisation complète.",
    ...
    },
    {
      ...
      tech: ["PHP", "MVC", "MySQL"],
      desc: "Application de gestion de recettes Cookify avec architecture MVC sécurisée.",
    ...
    },
    {
      ...
      desc: "Jeu web immersif The Lost avec mécaniques d'énigmes complexes et interface réactive.",
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-200 font-sans scroll-smooth selection:bg-white/10 overflow-x-hidden">
      ...
      {/* Message de Bienvenue */}
      <div className={`fixed left-6 bottom-6 z-[200] transition-all duration-1000 transform ${showWelcome ? 'translate-y-0 opacity-100 scale-100' : 'translate-y-20 opacity-0 scale-90'}`}>
        <div className="bg-slate-900 border border-white/20 p-5 rounded-2xl shadow-3xl flex items-center gap-4 max-w-sm backdrop-blur-2xl">
          <div className={`w-12 h-12 rounded-xl ${theme.primary} flex items-center justify-center text-white shadow-lg animate-pulse`}>
            <Sparkles size={24} />
          </div>
          <div>
            <h4 className="font-black text-sm text-white uppercase tracking-wider italic">Bienvenue !</h4>
            <p className="text-xs text-slate-400">Ravi de vous accueillir sur mon portfolio digital.</p>
          </div>
          <button onClick={() => setShowWelcome(false)} className="text-slate-500 hover:text-white transition-colors">
            <X size={18} />
          </button>
        </div>
      </div>
      ...
      <nav className={`fixed w-full top-0 z-[100] transition-all duration-300 ${scrolled ? 'bg-[#020617]/80 backdrop-blur-xl border-b border-white/5 py-4' : 'bg-transparent py-6'}`}>
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
          <div className="flex items-center gap-3 group cursor-pointer">
            ...
            <span className="text-xl font-bold tracking-tight text-white hidden sm:block uppercase italic">Mohamed Sanogo</span>
          </div>
      ...
      {/* Menu Mobile */}
      {isMenuOpen && (
        <div className="fixed inset-0 bg-[#020617] z-[110] flex flex-col items-center justify-center gap-8 md:hidden text-center p-6 backdrop-blur-2xl">
          <button onClick={() => setIsMenuOpen(false)} className="absolute top-8 right-8 text-slate-400 hover:text-white"><X size={40}/></button>
          {['Accueil', 'Projets', 'Profil', 'Contact'].map((item) => (
            <a key={item} href={`#${item.toLowerCase()}`} onClick={() => setIsMenuOpen(false)} className="text-4xl font-black text-white hover:text-blue-500 uppercase tracking-tighter transition-all">{item}</a>
          ))}
      ...
      <section id="accueil" className="relative pt-48 pb-32 px-6 flex items-center justify-center overflow-hidden min-h-screen">
        <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[600px] ${theme.primary} opacity-[0.03] blur-[120px] rounded-full -z-10`} />
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
          <div className="text-center lg:text-left">
            ...
            {/* --- NOM AVEC ANIMATION CONSTANTE --- */}
            <h1 className="text-5xl md:text-8xl font-black text-white mb-8 leading-[0.9] tracking-tighter uppercase">
              <span className="block animate-text-float">Mohamed</span>
              <br />
              <span className={`block animate-text-float delay-text ${theme.textLight} italic`}>Sanogo.</span>
            </h1>

            <p className="text-xl text-slate-400 mb-12 max-w-xl mx-auto lg:mx-0 leading-relaxed font-light italic">
              Conception d'architectures <strong>Web</strong> & <strong>IoT</strong>. 
          ...
          <div className="flex justify-center items-center">
            <div className="relative w-64 h-64 md:w-[450px] md:h-[450px] group">
              ...
              {/* Photo avec Animation de Flottaison (Couleurs Naturelles) */}
              <div className="relative w-full h-full bg-slate-900 rounded-[50px] border border-white/10 p-3 overflow-hidden shadow-2xl animate-float-photo">
                <img 
                  src={photoPath} 
                  alt="Mohamed Sanogo" 
              ...
              <div key={idx} className={`group flex flex-col bg-slate-900/40 backdrop-blur-sm rounded-[40px] border border-white/5 p-10 hover:${theme.border}/30 transition-all duration-500 shadow-2xl relative overflow-hidden`}>
                ...
                <div className={`p-4 rounded-2xl bg-white/5 border-l-4 ${theme.border} mb-8`}>
                   <div className="flex items-center gap-2 font-black text-white text-[10px] uppercase tracking-widest mb-1 italic">
                      <Award size={14} className={theme.text} /> Impact : {project.impact}
                   </div>
                </div>
                {/* --- BOUTON GITHUB RÉEL --- */}
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-20 items-center">
          ...
          <div className="text-center lg:text-left">
            <div className={`flex items-center justify-center lg:justify-start gap-3 ${theme.text} font-black text-xs uppercase tracking-[0.4em] mb-8 italic`}>
              <div className={`h-px w-8 ${theme.primary} hidden sm:block`} /> Dossier Académique
            </div>
            ...
            <p className="text-slate-400 text-lg mb-12 font-light italic leading-relaxed">Consultez mon CV interactif pour découvrir le détail de mes expériences.</p>
      ...
      <section id="contact" className="py-32 px-6">
        <div className="max-w-6xl mx-auto bg-slate-900/40 border border-white/10 p-10 md:p-20 rounded-[60px] relative overflow-hidden shadow-3xl backdrop-blur-md">
              ...
              <form className="space-y-6" onSubmit={handleContact}>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 uppercase text-[9px] font-black tracking-widest text-slate-500">
                  <div className="space-y-2">
                    ...
                    <input name="name" type="text" className="w-full bg-[#020617] border border-white/5 rounded-2xl px-6 py-5 focus:border-white outline-none text-white shadow-inner transition-colors" required />
                  </div>
                  <div className="space-y-2">
                    <label>Email Pro</label>
                    <input name="email" type="email" className="w-full bg-[#020617] border border-white/5 rounded-2xl px-6 py-5 focus:border-white outline-none text-white shadow-inner transition-colors" required />
                  </div>
                </div>
                <div className="space-y-2 uppercase text-[9px] font-black tracking-widest text-slate-500">
                  <label>Message</label>
                  <textarea name="message" rows="4" className="w-full bg-[#020617] border border-white/5 rounded-2xl px-6 py-5 focus:border-white outline-none text-white shadow-inner resize-none transition-colors" required></textarea>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <button type="submit" className={`w-full py-6 rounded-2xl font-black transition-all uppercase text-[10px] ${formStatus === 'success' ? 'bg-emerald-600 text-white' : `${theme.primary} text-white shadow-2xl`}`} disabled={formStatus === 'sending'}>
                    {formStatus === 'sending' ? "Envoi..." : formStatus === 'success' ? "Message Envoyé !" : "Envoyer"}
                  </button>
                  <a href={`mailto:${myEmail}?subject=Contact Portfolio`} className="w-full py-6 rounded-2xl font-black bg-white/5 border border-white/10 text-white text-[10px] uppercase block text-center shadow-inner">E-mail Direct</a>
                </div>
              </form>
            </div>
          </div>
